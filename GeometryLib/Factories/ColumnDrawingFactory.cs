...
{
    public class ColumnDrawingFactory
    {
        public ColumnDrawingFactory()
        {
        ...
        private void GetColumnDetailText(BlockTableRecord btr, Point3d basePoint, ColumnDrawInputArgs inputArgs)
        {
            ...
            //Prepare Second Line
            var secondLine = $"Boyuna Donatı Bilgisi ({12}f{inputArgs.LongBarDiameter})";
            GetTextToPoint(btr, new Point3d(stPoint.X, stPoint.Y - size - margin, stPoint.Z), secondLine, size);

            //Prepare Third Line
            var thirdLine = $"Etriye Donatı Bilgisi (Çap-Aralık-Sıklaştırma Aralığı) (f{inputArgs.LinkDiameter}/{inputArgs.LinkSpacing}/{inputArgs.LinkDenseSpacing})";
